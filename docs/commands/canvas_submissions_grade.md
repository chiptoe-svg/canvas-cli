...
```
  ...
  -h, --help                         help for grade
      ...
      --rubric stringArray           Rubric criterion score as <criterion-id>=<points>; repeat per criterion (criterion ids come from the assignment's rubric)
      --rubric-comment stringArray   Per-criterion comment as <criterion-id>=<text>; the criterion must also have a --rubric entry
