...
| `submissions` | `download` (all attempts), `prepare-pdfs --folder DIR --output DIR` (local-only PDF classification/page staging), `grade`, `bulk-grade --csv`, `comments`, `add-comment`, `delete-comment`, `missing` (read-only report), `excuse --student NAME\|ID --assignment NAME\|ID [--unexcuse]` (read-back) |
